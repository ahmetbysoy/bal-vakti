...
// src/api/* + src/api/lib/* kodlarını TEK DOSYAYA (api/index.js) birleştirir.
...
// src/api/lib modüllerini (sıralı) ve endpoint'leri derle
...
function build() {
  ...
  for (const f of LIB_ORDER) {
    libCode[f.replace('.js', '')] = readFileSync(path.join(ROOT, 'src/api/lib', f), 'utf8');
  }
  const handlerCode = {};
  for (const [name, f] of HANDLERS) {
    handlerCode[name] = readFileSync(path.join(ROOT, 'src/api', f), 'utf8');
  }
