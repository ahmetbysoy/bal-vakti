// 🗄️ Bal Vakti — veritabanı katmanı
// UPSTASH_REDIS_* env'leri tanımlıysa Upstash Redis kullanılır (Vercel + ücretsiz tier).
// Tanımlı değilse bellek modu devreye girer (yerel test için; Vercel'de veriler kaybolur!).

import { Redis } from '@upstash/redis';
import { DEFAULT_CONFIG } from './game.js';

const HAS_REDIS = !!(process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN);
const redis = HAS_REDIS
  ? new Redis({ url: process.env.UPSTASH_REDIS_REST_URL, token: process.env.UPSTASH_REDIS_REST_TOKEN })
  : null;

const mem = new Map();
const P = (k) => `balvakti:${k}`;

// @upstash/redis değerleri otomatik ayrıştırır (object döner);
// bellek modu string saklar. İkisini de güvenle işler.
function safeParse(v) {
  if (v == null) return null;
  if (typeof v === 'object') return v;
  try {
    return JSON.parse(v);
  } catch {
    return null;
  }
}

export function dbMode() {
  return redis ? 'upstash' : 'memory';
}

export async function getUser(id) {
  if (redis) {
    const v = await redis.get(P(`u:${id}`));
    return safeParse(v);
  }
  const v = mem.get(P(`u:${id}`));
  return safeParse(v);
}

export async function saveUser(id, state) {
  const s = JSON.stringify(state);
  if (redis) await redis.set(P(`u:${id}`), s);
  else mem.set(P(`u:${id}`), s);
}

// Referans kodları: kod → davet edenin id'si (1 yıl geçerli)
export async function getRef(code) {
  const v = redis ? await redis.get(P(`ref:${code}`)) : mem.get(P(`ref:${code}`));
  return v == null ? null : String(v); // "123" JSON otomatik sayıya dönebilir → string'e çevir
}
export async function setRef(code, id) {
  if (redis) await redis.set(P(`ref:${code}`), String(id), { ex: 60 * 60 * 24 * 365 });
  else mem.set(P(`ref:${code}`), String(id));
}

// Liderlik tablosu
export async function syncLb(id, name, score) {
  if (redis) {
    await redis.zadd(P('lb'), { score, member: String(id) });
    await redis.set(P(`name:${id}`), name || 'Anonim');
  } else {
    mem.set(P(`name:${id}`), name || 'Anonim');
    mem.set(P(`lb:${id}`), score);
  }
}

export async function topLb(n = 30) {
  if (redis) {
    // @upstash/redis: zrevrange yerine zrange(..., { rev: true, withScores: true })
    const rows = await redis.zrange(P('lb'), 0, n - 1, { rev: true, withScores: true });
    const arr = Array.isArray(rows) ? rows : [];
    const out = [];
    for (let i = 0; i + 1 < arr.length; i += 2) {
      const id = String(arr[i]);
      const name = (await redis.get(P(`name:${id}`))) || 'Anonim';
      out.push({ id, name, score: Number(arr[i + 1]) });
    }
    return out;
  }
  const arr = [...mem.entries()]
    .filter(([k]) => k.startsWith(P('lb:')))
    .map(([k, v]) => ({ id: k.slice(P('lb:').length), score: v }));
  return arr
    .sort((a, b) => b.score - a.score)
    .slice(0, n)
    .map((x) => ({ id: x.id, name: mem.get(P(`name:${x.id}`)) || 'Anonim', score: x.score }));
}

export async function myRank(id) {
  if (redis) {
    const r = await redis.zrevrank(P('lb'), String(id));
    return r === null ? null : r + 1;
  }
  const arr = [...mem.entries()]
    .filter(([k]) => k.startsWith(P('lb:')))
    .map(([k, v]) => ({ id: k.slice(P('lb:').length), score: v }))
    .sort((a, b) => b.score - a.score);
  const i = arr.findIndex((x) => x.id === String(id));
  return i < 0 ? null : i + 1;
}

// ── Canlı konfigürasyon (admin paneli) ──
export async function getConfig() {
  const base = { ...DEFAULT_CONFIG };
  if (redis) {
    const stored = await redis.get(P('cfg'));
    if (stored) return { ...base, ...safeParse(stored) };
    return base;
  }
  const stored = mem.get(P('cfg'));
  return { ...base, ...(stored ? safeParse(stored) : {}) };
}
export async function setConfig(partial) {
  const next = { ...(await getConfig()), ...partial };
  if (redis) await redis.set(P('cfg'), JSON.stringify(next));
  else mem.set(P('cfg'), JSON.stringify(next));
  return next;
}

// ── Admin oturumları (12 saat geçerli token) ──
export async function createSession(token, id) {
  if (redis) await redis.set(P(`adminsess:${token}`), String(id), { ex: 43200 });
  else mem.set(P(`adminsess:${token}`), String(id));
}
export async function getSession(token) {
  const v = redis ? await redis.get(P(`adminsess:${token}`)) : mem.get(P(`adminsess:${token}`));
  return v == null ? null : String(v);
}
export async function deleteSession(token) {
  if (redis) await redis.del(P(`adminsess:${token}`));
  else mem.delete(P(`adminsess:${token}`));
}

// ── Tüm oyuncu anahtarlarını tara (admin: arama, toplu bonus, istatistik) ──
export async function scanUserKeys(limit = 1000) {
  const prefix = P('u:');
  if (redis) {
    const keys = [];
    let cur = '0';
    do {
      const [nc, ks] = await redis.scan(cur, { match: `${prefix}*`, count: 100 });
      keys.push(...(Array.isArray(ks) ? ks : []));
      cur = nc;
    } while (cur !== '0' && keys.length < limit);
    return keys;
  }
  return [...mem.keys()].filter((k) => k.startsWith(prefix)).slice(0, limit);
}

export async function allUsers(limit = 1000) {
  const keys = await scanUserKeys(limit);
  if (redis && keys.length) {
    // ⚡ Pipeline: tüm GET'ler TEK HTTP isteğinde — Vercel timeout koruması
    const pipe = redis.pipeline();
    for (const k of keys) pipe.get(k);
    const vals = await pipe.exec();
    const out = [];
    for (let i = 0; i < keys.length; i++) {
      const st = safeParse(vals[i]);
      if (st) out.push({ id: keys[i].replace(P('u:'), ''), st });
    }
    return out;
  }
  const out = [];
  for (const k of keys) {
    const st = safeParse(mem.get(k));
    if (st) out.push({ id: k.replace(P('u:'), ''), st });
  }
  return out;
}

// ── Toplu istatistik (admin paneli) ──
export async function overview() {
  const users = await allUsers(2000);
  let totalBal = 0, totalEarned = 0, banned = 0, newToday = 0;
  const startOfDay = new Date();
  startOfDay.setHours(0, 0, 0, 0);
  for (const { st } of users) {
    totalBal += st.bal || 0;
    totalEarned += st.totalEarned || 0;
    if (st.banned) banned++;
    if ((st.created || 0) >= startOfDay.getTime()) newToday++;
  }
  return { players: users.length, totalBal, totalEarned, banned, newToday };
}

/* ═══════════════════ ⚔️ PvP — Bal Baskını kayıtları ═══════════════════ */

// Aktif saldırı: hedef başına tek saldırı olabilir
export async function getActiveRaid(targetId) {
  const v = redis ? await redis.get(P(`activeRaid:${targetId}`)) : mem.get(P(`activeRaid:${targetId}`));
  return v ? safeParse(v) : null;
}

// ⚡ TÜM aktif saldırıları TEK taramayla topla (bot beyni için kritik optimizasyon)
// Döner: { hedefId: raid, ... } — Redis istek sayısını dramatik azaltır
export async function allActiveRaids() {
  const map = {};
  if (redis) {
    const keys = [];
    let cur = '0';
    do {
      const [nc, ks] = await redis.scan(cur, { match: P('activeRaid:*'), count: 100 });
      keys.push(...(Array.isArray(ks) ? ks : []));
      cur = nc;
    } while (cur !== '0' && keys.length < 500);
    for (const k of keys) {
      const v = await redis.get(k);
      const r = safeParse(v);
      if (r) map[k.replace(P('activeRaid:'), '')] = r;
    }
    return map;
  }
  for (const [k, v] of mem.entries()) {
    if (k.startsWith(P('activeRaid:'))) {
      const r = safeParse(v);
      if (r) map[k.slice(P('activeRaid:').length)] = r;
    }
  }
  return map;
}
export async function setActiveRaid(targetId, raid) {
  const s = JSON.stringify(raid);
  if (redis) await redis.set(P(`activeRaid:${targetId}`), s);
  else mem.set(P(`activeRaid:${targetId}`), s);
}
export async function clearActiveRaid(targetId) {
  if (redis) await redis.del(P(`activeRaid:${targetId}`));
  else mem.delete(P(`activeRaid:${targetId}`));
}

// Kin listesi (kime saldırıldı kayıtları) — son 30
export async function addGrudge(targetId, entry) {
  const list = await getGrudges(targetId);
  list.unshift(entry);
  const trimmed = list.slice(0, 30);
  if (redis) await redis.set(P(`grudge:${targetId}`), JSON.stringify(trimmed));
  else mem.set(P(`grudge:${targetId}`), JSON.stringify(trimmed));
}
export async function getGrudges(targetId) {
  const v = redis ? await redis.get(P(`grudge:${targetId}`)) : mem.get(P(`grudge:${targetId}`));
  return v ? safeParse(v) : [];
}

// Savaş geçmişi (oyuncu başına) — son 20
export async function addRaidHist(userId, entry) {
  const list = await getRaidHist(userId);
  list.unshift(entry);
  const trimmed = list.slice(0, 20);
  if (redis) await redis.set(P(`hist:${userId}`), JSON.stringify(trimmed));
  else mem.set(P(`hist:${userId}`), JSON.stringify(trimmed));
}
export async function getRaidHist(userId) {
  const v = redis ? await redis.get(P(`hist:${userId}`)) : mem.get(P(`hist:${userId}`));
  return v ? safeParse(v) : [];
}

// Koalisyon sayacı: hedefe son 10 dk içinde kaç farklı saldırgan saldırdı
export async function recentRaiders(targetId, windowMs = 10 * 60 * 1000, now = Date.now()) {
  const gr = await getGrudges(targetId);
  const cutoff = now - windowMs;
  return [...new Set(gr.filter((g) => g.ts >= cutoff).map((g) => g.a))];
}

// Telegram bildirimi (bot token'ı varsa)
export async function tgNotify(userId, text) {
  const token = process.env.BOT_TOKEN;
  if (!token || !userId) return false;
  try {
    const r = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ chat_id: String(userId), text, parse_mode: 'HTML' }),
    });
    const j = await r.json();
    return !!j.ok;
  } catch (e) {
    return false;
  }
}

/* ═══════════════════ 🤖 YAPAY ZEKÂ ARICILAR (NPC botlar) ═══════════════════ */

// Bot meta kayıtları (balvakti:bot:<id>)
export async function listBots() {
  if (redis) {
    const keys = [];
    let cur = '0';
    do {
      const [nc, ks] = await redis.scan(cur, { match: P('bot:*'), count: 100 });
      keys.push(...(Array.isArray(ks) ? ks : []));
      cur = nc;
    } while (cur !== '0' && keys.length < 500);
    const out = [];
    for (const k of keys) {
      const v = await redis.get(k);
      const b = safeParse(v);
      if (b) out.push(b);
    }
    return out;
  }
  return [...mem.keys()]
    .filter((k) => k.startsWith(P('bot:')))
    .map((k) => safeParse(mem.get(k)))
    .filter(Boolean);
}
export async function getBot(id) {
  const v = redis ? await redis.get(P(`bot:${id}`)) : mem.get(P(`bot:${id}`));
  return v ? safeParse(v) : null;
}
export async function saveBot(bot) {
  const s = JSON.stringify(bot);
  if (redis) await redis.set(P(`bot:${bot.id}`), s);
  else mem.set(P(`bot:${bot.id}`), s);
}
export async function deleteBot(id) {
  if (redis) {
    await redis.del(P(`bot:${id}`));
    await redis.del(P(`u:${id}`));
    await redis.del(P(`name:${id}`));
    await redis.zrem(P('lb'), id);
  } else {
    mem.delete(P(`bot:${id}`));
    mem.delete(P(`u:${id}`));
    mem.delete(P(`name:${id}`));
    mem.delete(P(`lb:${id}`));
  }
}
export async function nextBotId() {
  if (redis) {
    const n = await redis.incr(P('botseq'));
    return `bot_${n}`;
  }
  const n = (mem.get(P('botseq')) || 0) + 1;
  mem.set(P('botseq'), n);
  return `bot_${n}`;
}

// Beyin turu kilidi: dünya turu 60 sn'de en fazla 1 kez çalışır
export async function brainLock(ttlSec = 60) {
  if (redis) {
    const ok = await redis.set(P('brainlock'), '1', { nx: true, ex: ttlSec });
    return ok !== null;
  }
  const last = mem.get(P('brainlock')) || 0;
  if (Date.now() - last < ttlSec * 1000) return false;
  mem.set(P('brainlock'), Date.now());
  return true;
}

// ── 🌍 Dünya olay akışı (son 30 olay) — savaş sekmesinde canlı yayın ──
export async function addEvent(entry) {
  const list = await getEvents();
  list.unshift({ ts: Date.now(), ...entry });
  const trimmed = list.slice(0, 30);
  if (redis) await redis.set(P('events'), JSON.stringify(trimmed));
  else mem.set(P('events'), JSON.stringify(trimmed));
}
export async function getEvents() {
  const v = redis ? await redis.get(P('events')) : mem.get(P('events'));
  return v ? safeParse(v) : [];
}
